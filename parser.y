...
/*íà÷àëî ñèíòàêñè÷åñêîãî äåðåâà*/
%start program

/*Äàëåå ñëåäóþò ïðàâèëà ãðàììàòèêè è äåéñòâèÿ*/
%%
	...
	if (tok)
	{
	  if (strchr(" (),&~|<=>.", *tok))
	  {
		...
		tok ++;
		  ...
		  if (c=='|') f--;//если с аргументами, то эту строчку убрать
		  //cout<<f<<endl;
	   if (c=='.')
		{
		   fl=0;
	  ...
	  if (str2[tok+strlen(tok)-str] != 0)
	  {
		...
		tok += strlen(tok)+1;
		//cout << c << " " << *tok << endl;
		if (c=='|' || c==')') f--; //если с аргументами, то эту строчку убрать
		return c;
	  }
